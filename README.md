...
- **选手排行** — 86 名选手可按赛事 / 种族 / 场次门槛筛选，按场次、胜负、胜率、ELO、ELO 净变排序
  ...
  - **日期时间段**：全部 / 近 7 天 / 近 30 天 / 近 90 天 / 近 1 年 / 自定义起止日，且可与赛事筛选叠加。
    ...
    该区间内没有出场的选手不会进入排行
- **选手详情** — 五项标签页
  - 总览：分赛事战绩、种族对抗、近期状态、**月度走势图**（柱=出场数，蓝线=当月胜率，橙虚线=月末 ELO，走右轴）
  - 地图：该选手在本赛季各张地图的场次与胜率
  - 对抗：对手种族对抗表 + 地图胜率分布
  - 对手：全部交手过的对手及其 H2H 战绩（点击进入双方对战），支持三语搜索，
    **表头可点击排序**（对手 / 种族 / 交手 / 胜 / 负 / 胜率 / 最近交手，同一列再点反向，默认数值列降序）
  - 对局记录：全部比赛，可按赛事筛选、按地图/对手搜索、分页浏览；
    排版与「双方对战 · 交手记录」一致 —— 单行 4 列（结果徽标 / 日期 / 对阵 / 地图）+ 两栏列优先
...
- **月末 ELO**（月度走势图的橙线）是从「官方 ELO」**倒推**出来的：`eloEnd(M) = 当前官方 ELO − Σ(月份晚于 M 的净变)`，
  末月必然等于官方 ELO，相邻两月的差恰好等于后一月的净变（`verify.mjs` 会校验这条）。
  注意：站点对尚未结算的对局不返回 `elo_delta`（如 2026-09 整月缺失），此时该月净变记 0，
  与「官方 ELO 也尚未变动」一致，不会凭空造出涨幅 —— 图表里这类月份的点是**虚线空心点**。
